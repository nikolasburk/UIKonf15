...
#define DEFAULT_MARGIN 8.0
#define TAB_BAR_HEIGHT 56.0

#define MIN_THUMBNAIL_DISTANCE 4.0
...
- (id)initWithFrame:(CGRect)frame startYear:(NSInteger)startYear endYear:(NSInteger)endYear skip:(NSRange)skip
{
    ...
    if (self)
    {
        _timeLineView = [[NBTimeLineView alloc] initWithStartYear:startYear endYear:endYear skip:skip width:self.frame.size.width type:TLV_YearsOnly];
...
- (void)buildTimeLineWithWidth:(CGFloat)width startYear:(NSInteger)startYear endYear:(NSInteger)endYear skip:(NSRange)skip
{
    ...
    _timeLineView = [[NBTimeLineView alloc] initWithStartYear:startYear endYear:endYear skip:skip width:self.frame.size.width - 2 * DEFAULT_MARGIN type:TLV_YearsOnly];
    self.timeLineView.layer.cornerRadius = 2.5;
...
- (void)setStories:(NSArray *)stories
{
    ...
    [self highlightStoryThumbnailAtIndex:0];
}
...
{
    StoryThumbnail *currentlyHighlightedStoryThumbnail = self.storyThumbnails[self.indexOfHighlightedThumbnail];
    [currentlyHighlightedStoryThumbnail toggleHighlighted];
}
...
- (void)highlightStoryThumbnailAtIndex:(NSInteger)index;
{
    ...
    StoryThumbnail *storyThumbnail = self.storyThumbnails[index];
    [storyThumbnail toggleHighlighted];
    
    self.indexOfHighlightedThumbnail = index;
}
