...
{
    if (_delegate && [_delegate respondsToSelector:@selector(gridView:willChangeGridPatternFromPattern:toPattern:)])
    {
        [_delegate gridView:self willChangeGridPatternFromPattern:_currentPattern toPattern:pattern];
    }
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(gridView:gridView:didDeselectStreamView:atIndex:)])
    {
        if (oldSelectedIndex < [_contentViews count])
        {
            [_delegate gridView:self didDeselectStreamView:_contentViews[oldSelectedIndex] atIndex:oldSelectedIndex];
        }
    }
    
    if (_delegate && [_delegate respondsToSelector:@selector(gridView:didSelectStreamView:atIndex:)])
    {
        [_delegate gridView:self didSelectStreamView:_contentViews[_selectedViewIndex] atIndex:_selectedViewIndex];
    }
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(gridView:didChangeGridPatternFromPattern:toPattern:)])
    {
        [_delegate gridView:self didChangeGridPatternFromPattern:_currentPattern toPattern:pattern];
    }
}
...
- (void)removeContentViewAtIndex:(NSInteger)index
{
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(gridView:willRemoveView:atIndex:)])
    {
        [_delegate gridView:self willRemoveView:viewToRemove atIndex:index];
    }
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(gridView:didRemoveView:atIndex:)])
    {
        [_delegate gridView:self didRemoveView:viewToRemove atIndex:index];
    }
}
...
- (void)setSelectedViewIndex:(NSInteger)selectedViewIndex
{
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(gridView:didDeselectStreamView:atIndex:)])
    {
        [_delegate gridView:self didDeselectStreamView:nil atIndex:oldIndex];
    }
...
#pragma warning StreamView
        ...
        if (_delegate && [_delegate respondsToSelector:@selector(gridView:didSelectStreamView:atIndex:)])
        {
            [_delegate gridView:self didSelectStreamView:(UIView *)touchedStreamView atIndex:_selectedViewIndex];
        }
        
        if (_delegate && [_delegate respondsToSelector:@selector(gridView:didDeselectStreamView:atIndex:)])
        {
            [_delegate gridView:self didDeselectStreamView:(UIView *)touchedStreamView atIndex:oldIndex];
        }
...
{
    //[NSObject cancelPreviousPerformRequestsWithTarget:self selector:@selector(gridView:startDragging:) object:nil];
    if (_isDragging)
    {
...
- (void)touchesEnded:(NSSet*)touches withEvent:(UIEvent*)event
{
    ...
    if (![_tapTimer isValid] && _isDragging)
    {
        ...
        {
            if (_delegate && [_delegate respondsToSelector:@selector(gridView:willExchangeView:atIndex:withView:atIndex:)]){
                [_delegate gridView:self willExchangeView:_contentViews[sourceIndex] atIndex:sourceIndex withView:_contentViews[targetIndex] atIndex:targetIndex];
            }
            ...
            if (_delegate && [_delegate respondsToSelector:@selector(gridView:didExchangeView:atIndex:withView:atIndex:)])
            {
                [_delegate gridView:self didExchangeView:_contentViews[sourceIndex] atIndex:sourceIndex withView:_contentViews[targetIndex] atIndex:targetIndex];
            }
        }
